...
from applyGeometricTransformation_Ski import *
        ...
        if (frameFound):                     # If frames remain in the video
            ...
            # If this frame is after the first frame in the video
                ...
                if (n_box == 1):  # For 1 Box
                    ...
                    Xs,Ys,newbbox = applyGeometricTransformation_Ski(startXs,startYs,newXs,newYs,np.squeeze(bbox),width,height)
                ...
                if (n_box == 2):  # For 2 Boxes
                    ...
                    # Final Transformation of Feature Positions and Box
                    Xs1,Ys1,newbbox[:,:,0] = applyGeometricTransformation_Ski(startXs1,startYs1,newXs1,newYs1,np.squeeze(bbox[:,:,0]),width,height)
                    Xs2,Ys2,newbbox[:,:,1] = applyGeometricTransformation_Ski(startXs2,startYs2,newXs2,newYs2,np.squeeze(bbox[:,:,1]),width,height)
